            ...
            <div class="row">
                <div class="col-lg-4">
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form action="{{ route('course-category.store') }}" method="post" autocomplete="off" enctype="multipart/form-data">
                                @csrf
                                <div class="mb-3">
                                        ...
                                        @if ($categories)
                                            @foreach($categories as $category)
                                                ...
                                                @if(count($category->subcategory))
                                                    @include('pages.admin.course-category.create-sub-category',['subcategories' => $category->subcategory])
                                                @endif
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <table class="table table-sm table-striped" id="crudTable">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($categories as $category)
                                            ...
                                            <td class="text-center">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu animated--fade-in-up" aria-labelledby="dropdownFadeInUp">
                                                        <a class="dropdown-item" href="{{ route('course-category.edit', $category->id) }}">Edit</a>
                                                        <form action="{{ route('course-category.destroy', $category->id) }}" method="POST" id="deleteForm{{ $category->id }}">
                                                            @method('delete')
                                                            @csrf
                                        ...
                                        @if(count($category->subcategory))
                                            @include('pages.admin.course-category.list-sub-category',['subcategories' => $category->subcategory])
                                        @endif
                                    @endforeach
                                </tbody>
                            </table>
